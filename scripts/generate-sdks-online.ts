...
async function main() {
  ...
  try {
    ...
    // const pullResponse = spawnSync("docker", [
    //   "pull",
    //   "cmgriffing/gilfold-openapi-client-generator:gilligan",
    // ]);

    console.log("Spawning Docker container");
    const runResponse = spawnSync("docker", [
      "run",
      "-d",
      "-p",
      "8888:8080",
      "cmgriffing/gilfold-openapi-client-generator:skipper",
    ]);

    CID = runResponse.output
      .toString()
      .replace("\n", "")
      .replace(new RegExp(",", "g"), "");

    console.log("Docker container created with ID: ", chalk.bgGreen(CID));
    ...
    if (CID) {
      ...
      spawnSync("docker", ["stop", CID]);
      console.log("Removing container");
      spawnSync("docker", ["rm", CID]);
    }
